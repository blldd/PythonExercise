...
class LVQ(object):
    def __init__(self, m, n, X, Y, alpha=0.1, max_iter=100):
        self.m = m
        self.n = n
        self.d = X.shape[1]
        self.size = m * n
        ...
        # self.output = np.random.rand(self.m, self.n)
        # self.output = self.output > 0.5
    ...
    def train(self):
        ...
        for it in range(self.max_iter):
            for idx, sample in enumerate(self.data):
                win_m, win_n, max_sim = self.find_winner(sample)
                if int(self.labels[idx]) == int(self.output[win_m, win_n]):
                    self.weights[win_m, win_n] += self.learning_rate(it, 0) * (sample - self.weights[win_m, win_n])
                else:
                    self.weights[win_m, win_n] -= self.learning_rate(it, 0) * (sample - self.weights[win_m, win_n])
...
if __name__ == '__main__':
    ...
    with open('F:/ECNU/Course/KnowledgeAna/results/lvq.res', 'a') as res:
        MAE_TOTAL = 0
        for QID in QID_LIST:
            print('LVQ for query ' + QID)
            vector = Vector(QID)
            ...
            lvq = LVQ(10, 10, X_train, Y_train, max_iter=100)
            print('clustering...')
