...
if __name__ == '__main__':
    ...
    with open('E:/ECNU/Course/KnowledgeAna/results/KMeans.res', 'a') as res:
        MAE_TOTAL = 0
        for QID in QID_LIST:
            print('KMeans for query ' + QID)
            ...
            train_set = dict()
            f = line_reader('E:/ECNU/Course/KnowledgeAna/tag_data_set/' + QID + '/train_set')
            ...
            #call sklearn.Lasso()
            clfRand = KMeans(n_clusters=2,random_state=0)
            clfRand.fit(X_train, Y_train)
            train_end_t = time.time()
            print('training finish, cost time: %d' % (train_end_t - train_start_t))

            test_set = dict()
            f = line_reader('E:/ECNU/Course/KnowledgeAna/tag_data_set/' + QID + '/test_set')
            line = next(f)
