...
class ReturnType(object):
    def __init__(self, all, fromHead, max):
        self.max_path_sum_all = all
        self.max_path_sum_head = fromHead
        self.max_val = max  # 防止节点全是负数，
...
class Solution(object):
    ...
    def max_sum_path(self, root):
        """
        求一棵二叉树中最大的路径和。
        该路径可以是二叉树中某一节点到树中任意一个节点的所经过的路径，不允许重复经过一个节点，不必经过根节点。
...
def process(head):
    if head == None:
        return ReturnType(0, 0, -sys.maxsize)

    # 黑盒
    left_data = process(head.left)
    right_data = process(head.right)

    max_val = max(head.val, max(left_data.max_val, right_data.max_val))

    max_path_sum_head = max(max(left_data.max_path_sum_head, right_data.max_path_sum_head) + head.val, head.val)

    max_path_sum_all = max(max(left_data.max_path_sum_all, right_data.max_path_sum_all), max_path_sum_head)

    return ReturnType(max_path_sum_all, max_path_sum_head, max_val)
...
if __name__ == '__main__':
    ...
    print(Solution().max_sum_path(head))
